...
export default function EditPreferencesScreen() {
  ...
  const customActionOptions: ActionOption[] = [
    {
      id: "allergy",
      label: t("profile.addAllergy" as any),
      icon: "exclamation-triangle",
      color: "red",
      onPress: () => {
        form.openCustomInput("allergy");
        setShowCustomInput(true);
      },
    },
    {
      id: "preference",
      label: t("profile.addPreference" as any),
      icon: "leaf",
      color: "green",
      onPress: () => {
        form.openCustomInput("preference");
        setShowCustomInput(true);
      },
    },
    {
      id: "cuisine",
      label: t("profile.addCuisine" as any),
      icon: "cutlery",
      color: "amber",
      onPress: () => {
        form.openCustomInput("cuisine");
        setShowCustomInput(true);
      },
    },
    {
      id: "equipment",
      label: t("profile.addEquipment" as any),
      icon: "wrench",
      color: "blue",
      onPress: () => {
        form.openCustomInput("equipment");
        setShowCustomInput(true);
      },
    },
  ];
    ...
    <View className="flex-1" style={{ backgroundColor: colors.card }}>
      ...
      >
        ...
        <View className="flex-1 bg-white dark:bg-gray-900">
          <KeyboardAvoidingView className="flex-1" behavior="padding">
            ...
            >
              {/* Global Search */}
              <SearchInput
                value={form.globalSearch}
                onChangeText={form.setGlobalSearch}
              ...
              <Section title={`⚠️ ${t("profile.allergies")}`} className="mb-4">
                <View className="flex-row flex-wrap gap-2 mt-2">
                  {form.filteredAllergies.map((item) => (
                    <Chip
                      ...
                      selected={form.isRestrictionSelected(item.id)}
                      onPress={() => form.toggleRestriction(item)}
                      size="sm"
                    />
                  ))}
                </View>
                {form.selectedRestrictions.filter(
                  (r) => r.isAllergy && r.customValue,
                ).length > 0 && (
                    ...
                    {form.selectedRestrictions
                      ...
                      .map((item) => (
                        <Chip
                          ...
                          onPress={() =>
                            form.setSelectedRestrictions((prev) =>
                              prev.map((r) =>
                                r.id === item.id
                ...
                {form.selectedRestrictions.filter(
                  (r) => r.isAllergy && r.isSelected,
                ).length > 0 && (
                  <Text className="text-xs text-gray-400 dark:text-gray-500 mt-2">
                    {t("profile.selectedCount", {
                      count: form.selectedRestrictions.filter(
                        (r) => r.isAllergy && r.isSelected,
                      ).length,
                    })}
                  </Text>
              ...
              {/* Cuisines */}
              <Section
                title={`🍳 ${t("profile.favoriteCuisines")}`}
                className="mb-4"
              >
                <View className="flex-row flex-wrap gap-2 mt-2">
                  {form.filteredCuisines.map((cuisine) => (
                    <Chip
                      ...
                      selected={form.isCuisineSelected(cuisine.id)}
                      onPress={() => form.toggleCuisine(cuisine.id)}
                      size="sm"
                    />
                  ))}
                </View>
                {form.selectedCuisines.filter((c) => c.customName).length >
                  0 && (
                  <View className="flex-row flex-wrap gap-2 mt-3">
                ...
                {form.selectedCuisines.filter((c) => c.isSelected).length >
                  0 && (
                  <Text className="text-xs text-gray-400 dark:text-gray-500 mt-2">
                    {t("profile.selectedCount", {
                      count: form.selectedCuisines.filter((c) => c.isSelected)
                        .length,
                    })}
                  </Text>
                )}
              ...
              <Section title={`🔧 ${t("profile.equipment")}`} className="mb-4">
                <View className="flex-row flex-wrap gap-2 mt-2">
                  {form.filteredEquipment.map((eq) => (
                    <Chip
                      ...
                      selected={form.isEquipmentSelected(eq.id)}
                      onPress={() => form.toggleEquipment(eq.id)}
                      size="sm"
                    />
                  ))}
                </View>
                {form.selectedEquipment.filter((e) => e.customName).length >
                  0 && (
                  <View className="flex-row flex-wrap gap-2 mt-3">
                    {form.selectedEquipment
                      ...
                      .map((item) => (
                        <Chip
                          ...
                          selected={item.isSelected}
                          onPress={() =>
                            form.setSelectedEquipment((prev) =>
                              prev.map((e) =>
                                e.id === item.id
                ...
                {form.selectedEquipment.filter((e) => e.isSelected).length >
                  0 && (
                  <Text className="text-xs text-gray-400 dark:text-gray-500 mt-2">
                    {t("profile.selectedCount", {
                      count: form.selectedEquipment.filter((e) => e.isSelected)
                        .length,
                    })}
                  </Text>
                )}
        ...
        <BottomSheet
          ...
          onOk={handleAddCustomAndClose}
        >
          <View className="gap-4 pb-4">
            <Text className="text-gray-600 dark:text-gray-400">
              {form.customType === "allergy" &&
                t("profile.addAllergyDesc" as any)}
              {form.customType === "preference" &&
                t("profile.addPreferenceDesc" as any)}
              {form.customType === "cuisine" &&
                t("profile.addCuisineDesc" as any)}
              {form.customType === "equipment" &&
                t("profile.addEquipmentDesc" as any)}
            </Text>
            <Input
              placeholder={t("profile.customValuePlaceholder" as any)}
              value={form.customValue}
              onChangeText={form.setCustomValue}
              showClearButton
            />
          </View>
        </BottomSheet>
