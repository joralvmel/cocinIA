...
import { WEEKLY_PLAN_AI_CONFIG, FALLBACK_MODELS } from '@/config';
...
function buildBasePreparationsPrompt(
  ...
  const t = (key: string, vars?: Record<string, unknown>) => tp(key, lang, vars);
  ...
  if (form.cuisines.length > 0) {
    parts.push(`${t('cuisineType')}: ${form.cuisines.map(resolveChipName).join(', ')}`);
  }
  if (form.equipment.length > 0) {
    parts.push(`${t('availableEquipment')}: ${form.equipment.map(resolveChipName).join(', ')}`);
  }

  if (form.ingredientsToInclude.length > 0) {
    parts.push(tp('batchKeyIngredients', lang, { list: form.ingredientsToInclude.join(', ') }));
  }
        ...
        if (p.recipe?.title && p.recipe?.ingredients) {
          try {
            ...
            try {
              ...
              if (__DEV__) console.warn(`Base prep "${p.name}" recipe could not be parsed`);
            }
          }
        }
        return {
...
function buildDayPlanSystemPrompt(
  ...
  if (allergies.length > 0) parts.push(`${t('allergiesWarning')}: ${allergies.join(', ')}`);
  if (preferences.length > 0) parts.push(`${t('dietaryPreferences')}: ${preferences.join(', ')}`);
  if (!allergies.length && !preferences.length) parts.push(t('noRestrictionsActive'));
  ...
  if (form.dailyCalorieTarget) {
    parts.push(tp('wpCalorieTarget', lang, { calories: form.dailyCalorieTarget }));
  }
  ...
  if (form.batchCookingEnabled && form.batchConfig) {
    parts.push(tp('wpBatchCookingMode', lang, { strategy: form.batchConfig.reuse_strategy }));
  ...
  // Response format
  parts.push('', tp('wpResponseFormat', lang), '', `[
  {
...
function buildDayUserPrompt(
  ...
  if (form.cookingTimeByMealType && Object.keys(form.cookingTimeByMealType).length > 0) {
    const details = mealTypes.map((mt) => `${mt}: ${form.cookingTimeByMealType[mt] ?? cookingTimeMinutes} min`).join(', ');
  ...
  // Include ingredients
  if (form.ingredientsToInclude.length > 0) {
    parts.push(tp('wpTryIngredients', lang, { list: form.ingredientsToInclude.join(', ') }));
  }

  // Favorite ingredients
  if (form.useFavoriteIngredients && favoriteIngredients.length > 0) {
    parts.push(tp('preferFavoriteIngredients', lang, { list: favoriteIngredients.join(', ') }));
  }
  ...
  if (form.routineMeals) {
    ...
    for (const mt of mealTypes) {
      ...
      if (routine) {
        const mealLabel = i18n.t(`weeklyPlan.mealTypes.${mt}`, { lng: lang }) as string;
  ...
  // Reinforce cuisines
  if (form.cuisines.length > 0) {
    parts.push(tp('wpCuisineReminder', lang, { list: form.cuisines.map(resolveChipName).join(', ') }));
  }

  // Special notes
  if (form.specialNotes) {
    parts.push(tp('wpSpecialNotesReminder', lang, { notes: form.specialNotes }));
  }

  // Excluded ingredients
  if (form.ingredientsToExclude.length > 0) {
    parts.push(tp('wpForbiddenIngredients', lang, { list: form.ingredientsToExclude.join(', ') }));
  }

  // Batch cooking assembly
  if (form.batchCookingEnabled && basePreparations.length > 0 && mealTypes.includes('lunch')) {
    const prepList = basePreparations.map((p, i) => `${i + 1}. ${p.name} (${p.type}): ${p.description}`).join('\n');

    const DAYS_ORDER: DayOfWeek[] = ['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday'];
    const sorted = form.selectedDays.slice().sort((a, b) => DAYS_ORDER.indexOf(a) - DAYS_ORDER.indexOf(b));
...
export const weeklyPlanGenerationService = {
  ...
  async generateWeeklyPlan(
    ...
    try {
      ...
      const DAYS_ORDER: DayOfWeek[] = ['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday'];
      const sortedDays = form.selectedDays.sort((a, b) => DAYS_ORDER.indexOf(a) - DAYS_ORDER.indexOf(b));
      ...
      for (const day of sortedDays) {
        ...
        const mealsToGenerate = [...dayConfig.meals].sort((a, b) => (MEAL_TYPE_ORDER[a] ?? 99) - (MEAL_TYPE_ORDER[b] ?? 99));
        if (mealsToGenerate.length === 0) continue;

        const userPrompt = buildDayUserPrompt(
          day, mealsToGenerate, dayConfig.cookingTimeMinutes,
          favoriteIngredients, form, generatedTitles.join(', '),
          normalizedLang, basePreparations,
        );
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
          try {
            if (attempt > 1) {
              ...
              console.log(`Day ${day} retry ${attempt}, waiting ${backoff}ms...`);
              await delay(backoff);
            }
            ...
            const rawResponse = (await model.generateContent(fullPrompt)).response.text();
            if (!rawResponse) { console.warn(`Day ${day} attempt ${attempt}: empty response`); continue; }
            ...
            if (validMeals.length === 0) { console.warn(`Day ${day} attempt ${attempt}: no valid meals`); continue; }
            ...
            if (dayMeals && dayMeals.length < mealsToGenerate.length) {
              console.warn(`Day ${day}: got ${dayMeals.length}/${mealsToGenerate.length} meals`);
            }
            break;
          } catch (parseError: any) {
            const msg = parseError?.message || String(parseError);
            const isServerError = msg.includes('503') || msg.includes('404') || msg.includes('429') || msg.includes('high demand') || msg.includes('no longer available') || msg.includes('rate');
            const errorType = msg.includes('503') ? '503' : msg.includes('404') ? '404' : msg.includes('429') ? '429' : 'parse';
            console.error(`Day ${day} attempt ${attempt} failed (${errorType}):`, msg.substring(0, 150));

            if (isServerError && currentModelIndex < FALLBACK_MODELS.length - 1) {
              currentModelIndex++;
              console.log(`Day ${day}: fallback to ${FALLBACK_MODELS[currentModelIndex]}`);
            }

            if (attempt === maxRetries) {
              if (dayMeals && dayMeals.length > 0) { console.warn(`Day ${day}: using partial meals`); break; }
              return { plan: null, success: false, error: `Failed to generate meals for ${day} after ${maxRetries} attempts` };
            }
          }
        }
      ...
      if (form.batchCookingEnabled && basePreparations.length > 0) {
        const lunchDays = sortedDays.filter((d) => form.dayConfigs[d]?.meals?.includes('lunch'));
        for (const prep of basePreparations) {
          if (prep.used_in_days.length === 0) prep.used_in_days = [...lunchDays];
      ...
      const plan: AIWeeklyPlanResponse = {
        ...
        total_estimated_calories: allMeals.reduce((sum, m) => sum + (m.estimated_calories || 0), 0),
        shopping_summary: [],
        tips: [],
      };
      ...
      return { plan: null, success: false, error: error instanceof Error ? error.message : 'Unknown error generating plan' };
    }
  },
  ...
  async regenerateSingleMeal(
    ...
    const existingTitles = currentPlanMeals.filter((m) => m.recipe?.title).map((m) => m.recipe.title);

    // Build a combined prompt
    const t = (key: string, vars?: Record<string, unknown>) => tp(key, normalizedLang, vars);
    ...
    const allergies = restrictions.filter((r) => r.is_allergy).map((r) => r.custom_value || r.restriction_type);
    const preferences = restrictions.filter((r) => !r.is_allergy).map((r) => r.custom_value || r.restriction_type);
    if (allergies.length > 0) systemParts.push(`${t('allergiesWarning')}: ${allergies.join(', ')}`);
    if (preferences.length > 0) systemParts.push(`${t('dietaryPreferences')}: ${preferences.join(', ')}`);
    if (!allergies.length && !preferences.length) systemParts.push(t('noRestrictionsActive'));

    if (profile?.country) systemParts.push(`${t('country')}: ${profile.country}`);
    if (profile?.currency) systemParts.push(`${t('currency')}: ${profile.currency}`);
    if (profile?.measurement_system) {
      systemParts.push(`${t('measurementSystem')}: ${profile.measurement_system === 'metric' ? t('metric') : t('imperial')}`);
    }

    systemParts.push('', t('jsonInstruction'), '', t('jsonStructure'), '', getJsonStructureExample(normalizedLang));
    ...
    userParts.push(tp('wpRegenMealInstruction', normalizedLang, { meal: mealName, day: dayName, minutes: cookingTimeMinutes }));

    if (existingTitles.length > 0) {
      userParts.push(tp('wpDoNotRepeat', normalizedLang, { titles: existingTitles.join(', ') }));
    }

    if (form.cuisines.length > 0) {
      userParts.push(`${t('cuisineType')}: ${form.cuisines.map(resolveChipName).join(', ')}`);
    }
    ...
    if (form.routineMeals) {
      ...
      if (routine) {
        userParts.push(tp('wpRoutineMealRotation', normalizedLang, { mealLabel: mealName, routine }));
      }
    }
    ...
    if (form.specialNotes) userParts.push(`${tp('specialNotesLabel', normalizedLang)}: ${form.specialNotes}`);
    if (form.servings && form.servings >= 1) userParts.push(`${t('servings')}: ${form.servings}`);

    if (form.ingredientsToExclude.length > 0) {
      userParts.push(tp('wpForbiddenIngredients', normalizedLang, { list: form.ingredientsToExclude.join(', ') }));
    }
      ...
      for (let attempt = 1; attempt <= 2; attempt++) {
        try {
          ...
          if (i === FALLBACK_MODELS.length - 1 && attempt === 2) {
            return { recipe: null, success: false, error: error instanceof Error ? error.message : 'Unknown error' };
          }
        }
      }
    }
